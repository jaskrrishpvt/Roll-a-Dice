...
const SecretForm: React.FC<SecretFormProps> = ({
  ...
  const handleButtonClick = (value: number) => {
    setSecret(value.toLocaleString());
  };

  if(RandomValue){
    setSecret(RandomValue.toLocaleString());
  }
...
{Dice && (
  <>
    <h1>Select a Secret Value</h1>
      <div>
        {[1, 2, 3, 4, 5, 6].map((value) => (
          <Button color='primary' key={value} onClick={() => handleButtonClick(value)}>
            {value}
          </Button>
        ))}
      </div>
